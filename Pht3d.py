...
class PHT3D:
    ...
    def calcNbParm(self):
        ...
        for n in kmin['rows']:
            ...
            for ip in range(1,len(kmin['cols'])): # compter le sortnb de parametres
    ...
    def updateDict(self,dic,old,kw,dbkw,lcols,lexclude=None):
        ...
        lrows=list(self.tempDbase[dbkw].keys());lrows2=[]
        if kw not in ['Rates','Kinetic_Minerals']: lrows.sort() #EV 04/01/22 from JG
        if lexclude!=None:
            for r in lrows:
    ...
    def importDB(self,fname):
        ...
        for li in f1:
            ...
            if curkw=='RATES': # kinetics
                ...
                if li2[:6]=='-start': # OA 15/7/22
                    cursp=bufr; bufr=''
                else:
                    bufr=bufr+li.split('#')[0].strip()
        ...
        for esp in elts:
            if len(esp.split('('))>1:
                ...
                if debut in master:
                    master.pop(debut);
        for n in ['E','O(-2)','Alkalinity','H(0)']:#OA 9/6/19  alk and h in list
