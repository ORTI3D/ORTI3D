...
class rflow():
    def __init__(self,core):
        self._lib=numpy.ctypeslib.load_library(core.baseDir+os.sep+'ilibq'+os.sep+'rflowC_dll','.')
        ti=numpy.ctypeslib.ndpointer(dtype = numpy.int_)
        tf=numpy.ctypeslib.ndpointer(dtype = numpy.float64,flags="C_CONTIGUOUS")
        self._lib.calcTube.argtypes = [tf,tf,C.c_int,C.c_int,tf,tf,C.c_int,C.c_int,tf,tf,tf,\
                              tf,tf,tf,tf,tf,ti]
        self._lib.calcTube.restype = C.c_void_p
    
        self._lib.ptsLigne.argtypes = [ti,tf,tf,tf,tf,tf,tf,tf,\
                                  tf,tf,tf,tf,tf]
        self._lib.ptsLigne.restype = C.c_void_p
    
        self._lib.calcPart.argtypes = [ti,tf,tf,tf,tf,\
                              tf,tf,tf,tf,ti]
        self._lib.calcPart.restype = C.c_void_p
    
        #self._lib.ptsLineP.argtypes = [ti,tf,tf,tf,tf,tf,tf,tf,tf,tf,ti]
        #self._lib.ptsLineP.restype = C.c_void_p
    
        self._lib.interp2d.argtypes = [ti,tf,tf,tf,tf,tf]
        self._lib.interp2d.restype = C.c_void_p
    
    def calcTube1(self,data,alpha0,vx,vy,xpin,ypin,clim):
    #if 3>2:
        for n in ['data','alpha0','vx','vy','xpin','ypin','clim']:
            exec('tmp = numpy.ascontiguousarray('+n+');'+n+'i = tmp.astype(numpy.float64)')
        ny, nx = vx.shape;
        np=len(xpin);nt=int(max(nx,ny)*1.7);ntmx=array([0]);
        tmp = numpy.asarray(ntmx);ntmxo=tmp.astype(numpy.int_);
        for n in ['xp','yp','tp','cu1','cu2']:
            exec(n+'o = numpy.empty([nt,np], dtype=numpy.float_, order =\'C\')')
        self._lib.calcTube(datai,alpha0i,nx,ny,vxi,vyi,nt,np,xpini,ypini,climi,\
    ...
    def ptsLigne(self,ind,tet,dc,xp,yp,tp,cu1,cu2):
    #if 3>2:
        ...
        self._lib.ptsLigne(indi,teti,dci,xpi,ypi,tpi,cu1i,cu2i,\
                  xpo,ypo,tpo,cu1o,cu2o)
        return xpo,ypo,tpo,cu1o,cu2o
    
    def calcParticule(self,data,vx,vy,clim):
    ...
    def ptsLigne2(self,xp,yp,tp,dt,zp=None):
        ...
        for n in ['xp','yp','tp','zp']:
            exec(n+'o = numpy.empty([nt], dtype=numpy.float_)')
        tmp = numpy.asarray(it);ito=tmp.astype(numpy.int_);
        self._lib.ptsLineP(dimsi,xpi,ypi,tpi,dti,zpi,xpo,ypo,tpo,zpo,ito)
    ...
    def interp2d(self,dims,data,xp,yp,zp):
        for n in ['data','xp','yp','zp']:
            exec('tmp = numpy.asarray('+n+');'+n+'i = tmp.astype(numpy.float_)')
        tmp = numpy.asarray(dims);dimsi=tmp.astype(numpy.int_);
        [nx,ny,np]=dims
        zpo=numpy.empty([ny,nx], dtype=numpy.float_)
        self._lib.interp2d(dimsi,datai,xpi,ypi,zpi,zpo)
        return zpo
